...
namespace nn {
    ...
    public:
        ...
        // Сумма по строкам (возвращает матрицу 1 x cols)
    ...
    // Математические функции активации
    namespace math {
        inline float sigmoid(float x) { return 1.0f / (1.0f + std::exp(-x)); }
        inline float sigmoid_deriv(float y) { return y * (1.0f - y); } // y - это уже активированное значение

        inline float relu(float x) { return x > 0 ? x : 0.0f; }
        inline float relu_deriv(float x) { return x > 0 ? 1.0f : 0.0f; } // x - это значение до активации

        inline float tanh_func(float x) { return std::tanh(x); }
        inline float tanh_deriv(float y) { return 1.0f - y * y; } // y - это уже активированное значение
    }
}